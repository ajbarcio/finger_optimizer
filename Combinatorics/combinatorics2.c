...
//     unsigned long numUnique = (unsigned long)pow(2,log2(numMatrices/2));

//     Set *s = create_set(numUnique);
//     printf("%ld \n", set_memory_usage(s));
//     unsigned char *testList = (unsigned char *)malloc(numMatrices*numBytes);
//     memset(testList, 0x00, numMatrices*numBytes);
//     structureMatrix m;
//     // unsigned long pattern = 0x123456789A;
//     // uint8_t column;
//     bool test;
//     for (uint8_t i = 0; i<50; i++) {
//         memset(m, 0x00, numBytes);
...
# endif // TRACE
    ...
    # ifdef TEST
        unsigned long numMatrices = 100000;
        unsigned long numUnique = (unsigned long)pow(2,(int)log2(numMatrices/2));
        // printf("%ld \n", numUnique);
    # else
        unsigned long numMatrices = pow(3, (NROWS*NCOLS));
        unsigned long numUnique   = (unsigned long)(pow(2,log2(numMatrices/1920*2)));
    # endif
    ...
    printf("highest number of unique matrices expected: %ld\n", numUnique);
    printf("entries allocated to Set:                   %ld\n", numUnique*2);
    Set *uniqueSet = create_set(numUnique*2);
    ...
    //prepare other variables we need
    numUnique = 0;
    structureMatrix rawMatrix;
    structureMatrix canonicalForm;
    bool dup;
    // for each encodedIndex (1-d representation of a 2-d structure matrix) of the list
    // of all combinations
    for (unsigned long encodedIndex = 0; encodedIndex < numMatrices; encodedIndex++)
    {
        ...
        // printf("--------------------------------------------\n");
        // printf("canoncializing matrix, %ld", encodedIndex);

        dup =  set_insert(uniqueSet, canonicalForm);
        numUnique = count_set(uniqueSet);


        // dup = isUnique(canonicalForm, numUnique, allUnique);
        // if (dup) {
        ...
        memcpy(combinationsList+(encodedIndex*numBytes), rawMatrix, sizeof(rawMatrix));
        if ((encodedIndex % 100000) == 0){
            printf("total processed %ld, unique found %ld \n", encodedIndex+1, numUnique);
            fflush(stdout);
        }
    } // end for encodedIndex
    // check size to make sure its consistent
    printf("%ld\n", numUnique);

    // Set uniqueSetCopy = uniqueSet;

    merge_set_into(uniqueSet, uniqueSet);
    numUnique = count_set(uniqueSet);
    ...
    write_out_set_values(uniqueSet, "unique4x5.out");
